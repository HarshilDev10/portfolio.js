  ...
  const experiences = [
    {
      ...
      description:
        "Developed a College Management System using HTML, CSS, JavaScript, PHP, and MySQL, featuring secure login portals for both students and teachers, and implemented functionalities for marks entry and comprehensive student records management.",
        ...
        <div className="grid md:grid-cols-1 gap-6 relative z-10">
          {experiences.map((exp, index) => (
            <motion.div
              ...
              className="relative bg-transparent backdrop-blur-lg border border-white/20 rounded-2xl p-6 shadow-lg transition-all duration-300 overflow-hidden"
            >
    ...
    View Certificate
              ...
              <div className="flex flex-wrap gap-2">
                {exp.technologies.map((tech, i) => (
                  <span
                    key={i}
                    className="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-sm text-gray-200 transition-colors duration-200 hover:bg-white/30"
                  >
                    {tech}
                  </span>
                ))}
